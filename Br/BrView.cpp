...
CBrView::CBrView()
{
	...
	{
		int nCodeWriter, nIndType, nNlType, nDriver, nEndian, nEndType;
		// Modified on Mar.24,'16
		// start
		int nWrite;
		GetCodeWriter(nCodeWriter, nIndType, nNlType, nDriver,
			nEndian, nEndType, nWrite);
		//GetCodeWriter(nCodeWriter, nIndType, nNlType, nDriver,
		//	nEndian, nEndType);
		// end
		...
		if (!nNoDispSrcLang) {
			CSrcLangDialog dlg;
			dlg.m_nLang = nCodeWriter;
			if (dlg.DoModal() == IDOK) {
				rm.WriteProfileDword("", "NoDispSrcLang", dlg.m_bNodisp);
				nCodeWriter = dlg.m_nLang;
			}
		}

		// Modified on Mar.24,'16
		// start
		//SetCodeWriter(nCodeWriter, nIndType, nNlType, nDriver,
		//	nEndian, nEndType);
		SetCodeWriter(nCodeWriter, nIndType, nNlType, nDriver,
			nEndian, nEndType, nWrite);
		// end
...
{
	CRegManager rm;
	nVarType = (int)rm.GetProfileDword("", "LoopIndexVarType", 1);
...
void CBrView::GetCodeWriter(int& nCodeType,
							int& nIndType, int& nNlType,
							// Modified on Mar.24,'16
							// start
							//int& nDriver, int& nEndian, int& nEndType)
							int& nDriver, int& nEndian, int& nEndType, int& nWrite)
							// end
{
	CRegManager rm;
	...
	nDriver   = (int)rm.GetProfileDword("", "Driver", 1);
	nEndian   = (int)rm.GetProfileDword("", "Endian", 0);
	nEndType  = (int)rm.GetProfileDword("", "EndianType", 0);
	// Appended on Mar.24,'16
	// start
	nWrite    = (int)rm.GetProfileDword("", "WriteFunc", 0);
	// end
}

void CBrView::SetCodeWriter(int nCodeType, int nIndType, int nNlType,
							// Modified on Mar.24,'16
							// start
							int nDriver, int nEndian, int nEndType, int nWrite)
							//int nDriver, int nEndian, int nEndType)
							// end
{
	...
	// start
	if (pApp->IsTrial()) {
		m_pCodeWriter->EnableWriteFunc(FALSE);
	}
	else if (nCodeType == 4) {	// for PHP
		m_pCodeWriter->EnableWriteFunc(FALSE);
	}
	else {
		m_pCodeWriter->EnableWriteFunc(nWrite == 1);
	}
	// end
	...
	// Appended on Mar.24,'16
	// start
	rm.WriteProfileDword("", "WriteFunc", nWrite);
	// end
}
...
void CBrView::DoSrcOptions()
{
	...
	// Appended on Mar.24,'16
	// start
	dlg.m_bWrite = m_pCodeWriter->IsWriteFuncEnabled();
	// end
	
	if (dlg.DoModal() == IDOK) {
		SetCodeWriter(dlg.m_nLang,
			dlg.m_nIndent, dlg.m_nNewLine,
			((dlg.m_bDriver) ? 1 : 0),
			((dlg.m_bEndian) ? 1 : 0),
			// Modified on Mar.24,'16
			// start
