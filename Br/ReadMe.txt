================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: Br プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した Br には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには Br アプリケーションを構成している各ファイ
ルの概要説明が含まれます。
...
Br.h
   ...
   CBrApp アプリケーション クラスを宣言します。

Br.cpp
   これは、アプリケーション クラス CBrApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\Br.ico
   ...
   アイコンはメイン リソース ファイル Br.rc に含まれていま
   す。
...
res\Toolbar.bmp
   ...
   プをリソース エディタを使用して編集し、Br.rc ファイル内
...
BrDoc.h, BrDoc.cpp - ドキュメント
   これらのファイルは CBrDoc クラスを含みます。特別なドキュメント 
   データを追加したり、ファイルを保存したり、
   (via CBrDoc::Serialize) を読み込んだりするには、これらのファイ
   ルを編集してください。
    ドキュメントには次の文字列が含まれます:
        ファイルの拡張子:             bfm
        ファイルの種類のID:           Br.Document
        メイン フレーム キャプション: Br
        ドキュメントの種類の名前:     Br
        フィルタ名:                   Binary Format Files (*.bfm)
        ファイルの新しい短い名前:     Br
        ファイルの種類の長い名前:     Br.Document

BrView.h, BrView.cpp - ドキュメントのビュー
   これらのファイルは CBrView クラスを含みます。
   CBrView オブジェクトは CBrDoc オブジェクトを表示す
   るのに使用されます。
...
StdAfx.h, StdAfx.cpp
   ...
   Br.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
