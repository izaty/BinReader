...
void CodeWriterForC::WriteDriver()
{
	...
	if (IsWriteFuncEnabled()) {
		...
		WriteLine(2, 1,			"fprintf(stderr, \"Usage: %%s inputfilename outputfilename\\n\", argv[0]);");
	}
	else {
		WriteLine(1, 1,		"if (argc != 2) {");
		WriteLine(2, 1,			"fprintf(stderr, \"Usage: %%s filename\\n\", argv[0]);");
	...
	// start
	if (IsWriteFuncEnabled()) {
		WriteLine(1, 2,	"err_code = writeBinary(argv[2]);");

		WriteLine(1, 1,	"if (err_code == -4) {");
		WriteLine(2, 1,		"fprintf(stderr, \"write error\\n\");");
		WriteLine(2, 1,		"return err_code;");
		WriteLine(1, 1,	"}");
...
{
	// Appended on Mar.20,'16
	// start
	WriteLine(0, 1, "void econv(void *ptr, size_t size, size_t n)");
	WriteLine(0, 1, "{");
	WriteLine(1, 1,		"int i, j;");
	WriteLine(1, 1,		"unsigned char *ptr_uc = (unsigned char *)ptr;");
	WriteLine(1, 2,		"unsigned char work[16];");

	WriteLine(1, 1,		"for (i = 0; i < n; i++) {");
	WriteLine(2, 1,			"int base = i * size;");
	WriteLine(2, 1,			"for (j = 0; j < size; j++) {");
	WriteLine(3, 1,				"work[size-j-1] = ptr_uc[base+j];");
	WriteLine(2, 1,			"}");
	WriteLine(2, 1,			"for (j = 0; j < size; j++) {");
	WriteLine(3, 1,				"ptr_uc[base+j] = work[j];");
	WriteLine(2, 1,			"}");
	WriteLine(1, 1,		"}");
	WriteLine(0, 2, "}");
	// end
	...
	WriteLine(2, 1,			"econv(ptr, size, n);");
	// end
	WriteLine(1, 1,		"}");
	WriteLine(1, 1,		"else {");
	WriteLine(2, 1,			"ret_code = fread(ptr, size, n, stream);");
	...
	if (IsWriteFuncEnabled()) {
		WriteEOL(1);
		WriteLine(0, 1, "size_t ewrite(void *ptr, size_t size, size_t n, FILE *stream)");
		WriteLine(0, 1, "{");
 		WriteLine(1, 1,		"size_t ret_code;");
		WriteLine(1, 1,		"if (size > 1) {");
		WriteLine(2, 1,			"econv(ptr, size, n);");
		WriteLine(2, 1,			"ret_code = fwrite(ptr, size, n, stream);");
		WriteLine(2, 1,			"econv(ptr, size, n);");
		WriteLine(1, 1,		"}");
		WriteLine(1, 1,		"else {");
		WriteLine(2, 1,			"ret_code = fwrite(ptr, size, n, stream);");
...
void CodeWriterForC::WriteClearFuncStart()
{
	WriteLine(0, 1, "void deleteContents()");
	WriteLine(0, 1, "{");
}

void CodeWriterForC::WriteClearFuncEnd()
{
	WriteLine(0, 1, "}");
}
...
// Appended on Mar.21,'16
// start
void CodeWriterForC::WriteWFuncStart()
{
	WriteLine(0, 1, "int writeBinary(char *filePath)");
	WriteLine(0, 1, "{");
}

void CodeWriterForC::WriteWFuncEnd()
{
	WriteLine(1, 1,		"return 0;");
	WriteLine(0, 1, "}");
}

void CodeWriterForC::WriteWFileOpen()
{
	WriteLine(1, 1,		"if ((fp = fopen(filePath, \"wb\")) == NULL) {");
	WriteLine(2, 1,			"return -5;");
	WriteLine(1, 1,		"}");
}

void CodeWriterForC::WriteWFileClose()
{
	WriteLine(1, 1,		"fclose(fp);");
}

void CodeWriterForC::WriteWrite(Variable& variable,
								int nLoopLevel, LPCTSTR lpszSize)
{
	int nPointer = variable.GetLoopLevel() - nLoopLevel;
	if (nPointer < 0) {
	...
	WriteIndent(nLoopLevel + 1);
	if (IsEndianConvEnabled()) {
		m_pFs->Write("if (ewrite(");
	}
	else {
		m_pFs->Write("if (fwrite(");
	}
	if (!nPointer) {
		m_pFs->Write("&");
	}
	...
	WriteLine(nLoopLevel + 2, 1, "return -4;");
	WriteLine(nLoopLevel + 1, 1, "}");
}

void CodeWriterForC::WriteWDeclareFileDesc()
{
	WriteLine(1, 1,		"FILE *fp;");
}
// end