								  ...
								  int nLoopLevel, LPCTSTR lpszSize)
{
	...
	/*
	{
		if (lpszSize != NULL) {
			WriteIndent(nLoopLevel + 2);
			m_pFs->Write("frameSize = %s * %d;", 
				lpszSize, variable.GetDataType().GetSize()); WriteEOL(1);
			WriteIndent(nLoopLevel + 2);
			m_pFs->Write("buffer = new byte[frameSize];"); WriteEOL(1);
			WriteIndent(nLoopLevel + 2);
			m_pFs->Write("dis.readFully(buffer);"); WriteEOL(1);
			WriteIndent(nLoopLevel + 2);
			m_pFs->Write("temp = new DataInputStream(new ByteArrayInputStream(buffer));"); WriteEOL(1);

			WriteLoopStart(nLoopLevel, lpszSize);
			WriteIndent(nLoopLevel + 3);
			if (IsEndianConvEnabled() && !variable.GetDataType().IsOneByte()) {
				m_pFs->Write("temp.readFully(work);");
				WriteVariable(variable, nLoopLevel + 1);
				m_pFs->Write(" = ByteBuffer.wrap(work).order(bo).get",
